...
@NgModule({
  declarations: [AppComponent, QqqComponent, QqqDirective, BtnComponent, IfifComponent], // 해당 모듈에 포함될 구성요소(컴포넌트나 디렉티브, 파이브)의 리스트를 선언한다.
  // exports: 해당 모듈의 컴포넌트나 디렉티브을 다른 모듈에서 사용하고 싶을 때 작성. AppModule(최상위 모듈)에는 존재하지 않는다.
  imports: [BrowserModule, AppRoutingModule], //  FormsModule, AppRoutingModule 등과 같은 외부 모듈(혹은 라이브러리)을 사용하고 싶을 때 추가한다. 외부 모듈은 앵귤러 모듈일수도 있고, 사용자가 정의한 모듈일수도 있다.
